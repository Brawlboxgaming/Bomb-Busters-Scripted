...
function onLoad()
    ...
    WebRequest.get("https://raw.githubusercontent.com/Brawlboxgaming/Bomb-Busters-Scripted/refs/heads/main/ColouredTokens.lua", function(e)
        ...
        elseif e.text ~= "" then
            allBagsBag = getObjectsWithTag("All")[1]
            if allBagsBag then
                local validationBag = getObjectsWithAllTags({"Validation", "Scripted", "Destroy"})[1]
                local attentionBag = getObjectsWithAllTags({"Attention", "Scripted", "Destroy"})[1]
                local warningBag = getObjectsWithAllTags({"Warning", "Scripted", "Destroy"})[1]
    ...
    -- Continue with next setup phase
...
function finalCleanup()
    ...
    if allBagsBag then
        ...
        for _, bag in ipairs(allModifiedBags) do
            bag.setPosition(allBagsBag.getPosition())
